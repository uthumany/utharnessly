...
Choose a provider and keep its key in the process environment. Utharness does not write API keys to its database or configuration files.
...
When `UTHARNESS_PROVIDER` is omitted, Utharness selects the first configured provider in this order: OpenRouter, OpenAI, Groq, Together, DeepSeek, then Fireworks. Set the provider explicitly when more than one key exists.
